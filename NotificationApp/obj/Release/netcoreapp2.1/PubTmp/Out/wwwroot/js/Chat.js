...
document.getElementById("GetRuchHour").addEventListener("click", event => {
    const user = document.getElementById("userName").value;
    const message = document.getElementById("userMessage").value;
    var price = connection.invoke("RequestRashHour", 3).then(function (result) {
        if (result) {
...
document.getElementById("GetTripCost").addEventListener("click", event => {
    ...
    obj.NormalKM = 50;
    obj.RusKM = 10;
    obj.TotalWaitingMinutes = 8;
    obj.IsAdditionalFees = true;
    obj.PickUpTime = "17:32:15";
    obj.DropOffTime = "17:42:17";
    obj.WeekDay = 5;


    connection.invoke("RequestPriceCost", obj, 78).then(function (result) {
...
connection.on("NotifiedClientTripCost", (TripCost, TotalWaitingMinutes,NormalKM, RusKM) => { //Driver 
    // alert("");
    alert("push to   client with trip cost:" + TripCost + " " + "TotalWaitingMinutes : " + TotalWaitingMinutes);
...
document.getElementById("TripChat").addEventListener("click", event => {
    ...
    connection.invoke("SendMessage", 5, userIdFrom, userIdTo, message, userName, userType).catch(err => console.error(err.toString()));
    event.preventDefault();
});
//This method receive the message and Append to our list
connection.on("ReceiveMessage", (Trip_Id, User_Id_From, UserId_To, Message, UserName, UserType) => {
